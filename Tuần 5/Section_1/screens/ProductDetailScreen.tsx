...
import { phoneData, phoneColors } from "../data/dummy-data";
...
const ProductDetailScreen = ({ navigation, route }: Props) => {
  ...
  const [selectedColor, setSelectedColor] = useState<Color>(phoneColors[0]);

  useEffect(() => {
    if (selectedColorId) {
      const color = phoneColors.find((c) => c.id === selectedColorId);
      if (color) {
